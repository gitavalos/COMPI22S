...
Public Class MyParser
    ...
    Public Function GENERAR_RANGO(ByVal a As String, ByVal b As String) As NODO
        Dim temporal As New NODO
        Dim inicio As Integer = Asc(a)
        Dim final As Integer = Asc(b)
        If (final > inicio) Then
            For i As Integer = inicio To final  'se crean los nodos
                Dim ttemporal As New NODO
                ttemporal.correlativo = CONTADOR
                ttemporal.id = "RANGO"
                ttemporal.layer = Chr(i)
                CONTADOR = CONTADOR + 1

                Dim punto As New NODO
                punto.correlativo = CONTADOR
                punto.id = "O"
                punto.layer = "|"
                CONTADOR = CONTADOR + 1
                If Not (IsNothing(temporal.hijo_izquierdo)) Then
                    temporal.hijo_derecho = ttemporal
                    punto.hijo_izquierdo = temporal
                Else
                    punto.hijo_izquierdo = ttemporal
                End If
                temporal = punto
            Next
        End If
        Return temporal
    End Function
    Public Function COPIA_NODOS(ByVal RAIZ As NODO) As NODO
        Dim TEMPORAL As New NODO
        Dim B1 As Boolean = False
        Dim B2 As Boolean = False
        If Not (IsNothing(RAIZ.hijo_izquierdo)) Then
            TEMPORAL.hijo_izquierdo = COPIA_NODOS(RAIZ.hijo_izquierdo)
            B1 = True
        End If
        If Not (IsNothing(RAIZ.hijo_derecho)) Then
            B2 = True
            TEMPORAL.hijo_derecho = COPIA_NODOS(RAIZ.hijo_derecho)
        End If

        TEMPORAL.correlativo = CONTADOR
        TEMPORAL.id = RAIZ.id
        TEMPORAL.layer = RAIZ.layer
        TEMPORAL.NUMERO = RAIZ.NUMERO
        CONTADOR = CONTADOR + 1

        Return TEMPORAL
    End Function
    Public Function CONSULTA_TABLA(ByVal ID As String) As NODO
        Dim temporal As New NODO
        For Each ITEM As LISTA_NODO In LISTA
            If (ITEM.ID = ID) Then
                Return ITEM.RAIZ
            End If
        Next
        Return temporal
    End Function
    Public Function OBTENER_STRING(ByVal PADRE As NODO) As String
        ...
        With Reduction
            Select Case .Parent.TableIndex
                ...
                Case ProductionIndex.Regular_Minusgt_Newline
                    ...
                    Dim nuevo As New LISTA_NODO
                    nuevo.ID = OBTENER_STRING(.Item(0).Data)
                    nuevo.RAIZ = temporal
                ...
                Case ProductionIndex.A_Lbrace_Rbrace
                    ' <A> ::= '{' <REP_MACRO> '}' 
                    Dim temporal As New NODO
                    Dim ID As String = OBTENER_STRING(.Item(1).Data)
                    temporal = CONSULTA_TABLA(ID)
                    temporal = COPIA_NODOS(temporal.hijo_izquierdo)
                    Return temporal
                ...
                Case ProductionIndex.Guion_Digito_Minus_Digito
                    ...
                    Dim hijo1 As New NODO
                    hijo1 = GENERAR_RANGO(.Item(1).Data, .Item(3).Data)
                    hijo1 = hijo1.hijo_izquierdo
                ...
                Case ProductionIndex.Guion_Digito_Minus_Digito2
                    ' <GUION> ::= DIGITO '-' DIGITO 
                    Dim hijo1 As New NODO
                    hijo1 = GENERAR_RANGO(.Item(0).Data, .Item(2).Data)
                    hijo1 = hijo1.hijo_izquierdo
                    Return hijo1
